...
## 2026-08-17 — Guest-data privacy: private assets stay out of the public repo
- **Decision:** The guest list, nominative seating plan, and the couple's playlist are
  **never committed** to this repo. They live in the session scratchpad and are delivered
  to the client via chat file upload only.
- **Why:** This repo is **public** (it serves the live GitHub Pages site). Committing guest
  names would expose them publicly and permanently in git history.
- **If tracking is wanted:** use a **separate private repo**. Making *this* repo private is
  not a free-tier option for a Pages site — it would take the live site offline (Pages from
  private repos needs a paid GitHub plan). See learnings.md.
- **Inventory + policy:** `.claude/memory/deliverables.md`.

## 2026-08-17 — Print & event deliverables built with a Python stack
- **Decision:** All printables (faire-part variants, thank-you card, 24 table cards, envelope
  stickers, perfume coupon, logo/drawing exports) are generated by scripts in `assets/print/`
  using **reportlab + PyMuPDF (fitz) + Pillow**, IBM Plex Serif, the sage palette.
- **Conventions:** A7 = A5 at exact ½ scale (identical design); illustrations auto-cropped to
  visible content (`visible_bbox()`, numpy) with CV-verified centering; hand-drawn "sketch"
  motifs via a roughened-stroke `Pen` (`sketch.py`). Scripts are the source of truth — re-run
  to regenerate; commit script + output together.
- **Why:** Deterministic, versionable, no design-tool lock-in; matches the site's identity.
